...
def show_kline_data(params, pos):
    param = params[0]
    if param.data[4]:
      return "date = " + param.name + "<br/>" + "open = " + param.data[1] + "<br/>" + "high = " + param.data[
...
kline = Kline(subtitle=Index['desc'], subtitle_color =  '#008', subtitle_text_size=12, title_pos='center')
...
# 图合并到一张图表中
grid = Grid(height=600, width=1300, page_title=(Id[2:]+' : '+Index['name'][0]))
