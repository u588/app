...
def get_ic(stock_list, sql_name):
    ...
    for i in tqdm(stock_list):
        ...
        tempdf = tempdf[tempdf['分类标准编码'].isin(['008001','008003','008014'])].sort_values(by='分类标准编码')
    ...
    ddf.drop_duplicates(subset=(['StockCode','ICS']),keep='last').set_index('StockCode').to_sql(sql_name, engB, if_exists='replace')

get_ic(StockLists,"akStockIC")
engB.dispose()
engS.dispose()
