"""
AiStock V8 TDXAdapter — 通达信双端口数据适配器
...
路由规则:
  ...
  future_zz/dl/sh/gz/zj, option_zj/sh/sz, macro → 扩展端口
"""
...
# ─── Connection pool from base_services ──────────────────────────────────────
try:
    from base_services.connection_pool import TDXConnectionPool
except ImportError:
...
@dataclass
class _SimplePool:
    """简单连接池 — 当 base_services.TDXConnectionPool 不可用时使用"""
...
class TDXAdapter:
    ...
    def __init__(
        ...
        self._std_pool = _SimplePool(
            host=std_host, port=std_port,
            api_type="standard", max_conn=pool_size,
        )
        self._ext_pool = _SimplePool(
            host=ext_host, port=ext_port,
            api_type="extension", max_conn=pool_size,
        )
