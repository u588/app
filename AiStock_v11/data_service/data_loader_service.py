...
class DataLoaderService:
    ...
    def _load_config(self):
        ...
        # ─── 海外期货配置 ─────────────────────────────────────────────────
        # V11修正: core/extended_symbols 留空使用 ak_adapter 内置默认值
        # auxiliary_types 与 AUXILIARY_DATA_TYPES 对齐
        self._overseas_config: Dict[str, Any] = self._get_config_dict(
            ...
            default={
                ...
                "auxiliary_types": [
                    "cftc", "lme_stock", "lme_holding", "bond_zh_us",
                    "bond_us_10y", "eia_crude", "ism_pmi", "global_index",
                ],
            },
        )

        # ─── 宏观数据配置 ─────────────────────────────────────────────────
        # V11修正: Market 50 不支持 CPI/PPI/PMI, 改用 Market 38 扩展端口代码
        ...
        try:
            ...
            for _, row in calls.iterrows():
                ...
                try:
                    ...
                    if not df.empty:
                        latest = df.iloc[-1]
                        # 尝试从K线数据提取持仓量(oi), 字段名可能为 open_interest 或 hold
                        oi_val = 0
                        if "open_interest" in latest.index:
                            oi_val = int(latest.get("open_interest", 0))
                        elif "hold" in latest.index:
                            oi_val = int(latest.get("hold", 0))
                        entry = {
                            "code": code,
    ...
    def _load_macro_data(self) -> Dict[str, pd.DataFrame]:
        ...
        # V11修正: 默认使用 Market 38 代码, 与 codes.yaml 对齐
    ...
    def _load_macro_valuation_data(self) -> Dict[str, Any]:
        """加载宏观估值指标 (TDX扩展端口 Market 38 + PostgreSQL)

        V11 NEW: 宏观估值数据段
        包含: PMI(3_PMI/3_PMIH)、社融(5_TRY/5_TRL)、
              流动性(5_CNTY/5_SHS3M/8_ATY)、通胀(2_CPI/2_PPI)、
              估值(PE/PB百分位, 来自PostgreSQL)
        """
        start_time = time.time()
        result: Dict[str, Any] = {}

        # 1. 加载宏观指标 (TDX扩展端口)
        macro_indicators = self._macro_valuation_config.get("macro_indicators", [])
        ...
        for idx, cfg in enumerate(macro_indicators):
            ...
            try:
                df = self._tdx.get_macro_data(
                    code=code,
                    market=market,
                    count=60,  # 宏观数据月频为主, 取60条约5年
                )
                macro_data[sub_signal] = df
                if df.empty:
                    logger.warning("宏观估值指标 %s (%s) 数据为空", name, code)
            except Exception as e:
                logger.error("宏观估值指标 %s (%s) 加载失败: %s", name, code, e)
                macro_data[sub_signal] = pd.DataFrame()

            progress = (idx + 1) / (total + 1)
            self._notify_progress(DataSection.MACRO_VALUATION_DATA, progress, f"{name} 完成")

        result["macro_indicators"] = macro_data

        # 2. 加载估值数据 (PostgreSQL)
        val_codes = self._macro_valuation_config.get("valuation_codes", ["000300", "000905", "000852"])
        valuation_data: Dict[str, pd.DataFrame] = {}

        try:
            batch_result = self._db.get_index_pe_batch(val_codes)
            valuation_data = batch_result
        except Exception as e:
            logger.error("宏观估值段-批量估值数据加载失败: %s", e)
            for code in val_codes:
                try:
                    df = self._db.get_index_pe(code, 500)
                    valuation_data[code] = df
                except Exception as e2:
                    logger.error("估值数据 %s 加载失败: %s", code, e2)
                    valuation_data[code] = pd.DataFrame()

        result["valuation_data"] = valuation_data

        report = _validate_dict_data(macro_data, DataSection.MACRO_VALUATION_DATA)
        report.load_time_ms = (time.time() - start_time) * 1000
        self._quality_reports[DataSection.MACRO_VALUATION_DATA] = report
