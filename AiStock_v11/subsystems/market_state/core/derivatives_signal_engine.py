...
if TYPE_CHECKING:
    from .fund_flow_engine import FundFlowSignal
    from .option_pcr_engine import OptionPCRSignal
    from .macro_valuation_engine import MacroValuationSignal
    from .style_rotation_engine import StyleRotationSignal
...
class DerivativesResult:
    ...
    # ─── V11 新增4分量 ──────────────────────────────────────────────
    fund_flow_signal: Optional[Any] = None          # FundFlowSignal
    option_pcr_signal: Optional[Any] = None         # OptionPCRSignal
    macro_valuation_signal: Optional[Any] = None     # MacroValuationSignal
    style_rotation_signal: Optional[Any] = None     # StyleRotationSignal
    # ─── 综合结果 ───────────────────────────────────────────────────
    composite_signal: float = 0.0
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "fund_flow_signal": self.fund_flow_signal.to_dict() if self.fund_flow_signal else None,
            "option_pcr_signal": self.option_pcr_signal.to_dict() if self.option_pcr_signal else None,
            "macro_valuation_signal": self.macro_valuation_signal.to_dict() if self.macro_valuation_signal else None,
            "style_rotation_signal": self.style_rotation_signal.to_dict() if self.style_rotation_signal else None,
            "composite_signal": round(self.composite_signal, 2),
...
class DerivativesSignalEngine:
    ...
    def __init__(
        ...
        self._logger.info(
            "DerivativesSignalEngine V11 初始化完成 | "
    ...
    def calculate_all(
        self,
        fund_flow_signal: Optional[Any] = None,
        option_pcr_signal: Optional[Any] = None,
        macro_valuation_signal: Optional[Any] = None,
        style_rotation_signal: Optional[Any] = None,
    ) -> DerivativesResult:
        """全量计算衍生品信号

        V11: 接受4个新分量信号作为参数 (由 MarketStateEngine 注入)

        Args:
            fund_flow_signal: FundFlowSignal 实例
            option_pcr_signal: OptionPCRSignal 实例
            macro_valuation_signal: MacroValuationSignal 实例
            style_rotation_signal: StyleRotationSignal 实例
        """
        ...
        ff_status = "可用" if fund_flow_signal and fund_flow_signal.data_available else "不可用"
        pcr_status = "可用" if option_pcr_signal and option_pcr_signal.data_available else "不可用"
        mv_status = "可用" if macro_valuation_signal and macro_valuation_signal.data_available else "不可用"
        sr_status = "可用" if style_rotation_signal and style_rotation_signal.data_available else "不可用"
        self._logger.info(
            "衍生品信号全量计算完成: 综合=%.1f [%s] | "
            "商品=%d 期限=%d 基差=%d 行业=%d 海外=%s "
            "资金流=%s PCR=%s 宏观估值=%s 轮动=%s | %.0fms",
            ...
            ff_status, pcr_status, mv_status, sr_status,
            elapsed,
        )
    ...
    def generate_enhanced_report(self, result: Optional[DerivativesResult] = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            # ─── V11 新增3分量 ──────────────────────────────────────
            "fund_flow": result.fund_flow_signal.to_dict() if result.fund_flow_signal else None,
            "option_pcr": result.option_pcr_signal.to_dict() if result.option_pcr_signal else None,
            "macro_valuation": result.macro_valuation_signal.to_dict() if result.macro_valuation_signal else None,
            "style_rotation": result.style_rotation_signal.to_dict() if result.style_rotation_signal else None,
            # ─── 辅助信息 (不计入7分量) ─────────────────────────────
    ...
    def _calculate_domestic_composite(
        ...
        # ─── 4个新分量 (可能为None, 降级为0) ─────────────────────────
        fund_flow_val = fund_flow_signal.composite_signal if fund_flow_signal and fund_flow_signal.data_available else 0.0
        option_pcr_val = option_pcr_signal.composite_signal if option_pcr_signal and option_pcr_signal.data_available else 0.0
