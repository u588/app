...
class PlotlyVisualizer:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # 6. 行业/风格/规模轮动仪表板 (V11 NEW)
    # ═══════════════════════════════════════════════════════════════════════

    def plot_style_rotation_dashboard(
        self,
        style_rotation_data: Dict[str, Any],
    ) -> go.Figure:
        """绘制行业/风格/规模轮动交互仪表板

        Args:
            style_rotation_data: StyleRotationSignal.to_dict() 的输出
        """
        industry_rotation = _safe_get(style_rotation_data, "industry_rotation", {})
        style_signal = _safe_get(style_rotation_data, "style_signal", 0)
        style_direction = _safe_get(style_rotation_data, "style_direction", "neutral")
        size_signal = _safe_get(style_rotation_data, "size_signal", 0)
        size_direction = _safe_get(style_rotation_data, "size_direction", "neutral")
        heatmap_ranking = _safe_get(style_rotation_data, "industry_heatmap_ranking", [])
        rotation_alerts = _safe_get(style_rotation_data, "rotation_alerts", [])
        composite_signal = _safe_get(style_rotation_data, "composite_signal", 0)
        composite_direction = _safe_get(style_rotation_data, "composite_direction", "neutral")
        ...
        fig = make_subplots(
            ...
            subplot_titles=["行业动量排名", "风格轮动 (成长vs价值)", "轮动综合信号",
                           "规模轮动 (大盘vs小盘)", "行业热力图", "轮动预警"],
        ...
        if heatmap_ranking:
            ...
            sorted_ranks = sorted(heatmap_ranking, key=lambda x: x[1], reverse=True)
            industries = [r[0] for r in sorted_ranks]
            signals = [r[1] for r in sorted_ranks]
            bar_colors = [COLORS["primary"] if s > 0 else COLORS["danger"] for s in signals]

            fig.add_trace(go.Bar(
                y=industries, x=signals,
                orientation="h",
                marker=dict(color=bar_colors, opacity=0.85),
                name="行业动量",
                hovertemplate="<b>%{y}</b><br>信号: %{x:.1f}<extra></extra>",
            ), row=1, col=1)
            fig.add_vline(x=0, line_dash="dash", line_color=COLORS["neutral"], line_width=1, row=1, col=1)

        # ── 2. 风格轮动仪表 ──
        style_labels = ["成长", "中性", "价值"]
        style_values = [max(0, style_signal), 0, max(0, -style_signal)]
        style_colors_list = [COLORS["primary"], COLORS["neutral"], COLORS["danger"]]

        fig.add_trace(go.Bar(
            x=["成长领先", "价值领先"],
            y=[style_signal, -style_signal],
            marker=dict(color=[COLORS["primary"] if style_signal > 0 else COLORS["neutral"],
                              COLORS["danger"] if -style_signal > 0 else COLORS["neutral"]], opacity=0.85),
            name="风格信号",
            hovertemplate="<b>%{x}</b><br>信号: %{y:.1f}<extra></extra>",
        ), row=1, col=2)
        fig.add_hline(y=0, line_dash="dash", line_color=COLORS["neutral"], line_width=1, row=1, col=2)

        # ── 3. 综合信号仪表 ──
        gauge_color = _score_to_color((composite_signal + 100) / 2)
        fig.add_trace(go.Indicator(
            mode="gauge+number",
            value=composite_signal,
            number=dict(font=dict(size=28, color=gauge_color)),
            gauge=dict(
                axis=dict(range=[-100, 100], tickwidth=1),
                bar=dict(color=gauge_color, thickness=0.3),
                bgcolor="white",
                steps=[
                    dict(range=[-100, -30], color="#FECACA"),
                    dict(range=[-30, -10], color="#FEF08A"),
                    dict(range=[-10, 10], color="#F0FDF4"),
                    dict(range=[10, 30], color="#D9F99D"),
                    dict(range=[30, 100], color="#86EFAC"),
                ],
            ),
            title=dict(text=f"<b>{composite_direction}</b>", font=dict(size=12)),
        ), row=1, col=3)

        # ── 4. 规模轮动 ──
        fig.add_trace(go.Bar(
            x=["大盘领先", "小盘领先"],
            y=[size_signal, -size_signal],
            marker=dict(color=[COLORS["primary"] if size_signal > 0 else COLORS["neutral"],
                              COLORS["warning"] if -size_signal > 0 else COLORS["neutral"]], opacity=0.85),
            name="规模信号",
            hovertemplate="<b>%{x}</b><br>信号: %{y:.1f}<extra></extra>",
        ), row=2, col=1)
        fig.add_hline(y=0, line_dash="dash", line_color=COLORS["neutral"], line_width=1, row=2, col=1)

        # ── 5. 行业热力图 (scatter for heatmap) ──
        if industry_rotation:
            ind_names = []
            ind_mom5 = []
            ind_mom20 = []
            ind_colors = []
            for key, data in industry_rotation.items():
                if isinstance(data, dict):
                    ind_names.append(data.get("name", key))
                    ind_mom5.append(data.get("momentum_5d", 0))
                    ind_mom20.append(data.get("momentum_20d", 0))
                    sig = data.get("signal", 0)
                    ind_colors.append(COLORS["primary"] if sig > 0 else COLORS["danger"] if sig < 0 else COLORS["neutral"])

            if ind_names:
                fig.add_trace(go.Scatter(
                    x=ind_mom20, y=ind_mom5,
                    mode="markers+text",
                    text=ind_names,
                    textposition="top center",
                    textfont=dict(size=9),
                    marker=dict(size=14, color=ind_colors, opacity=0.85,
                               line=dict(width=1, color="white")),
                    name="行业热力",
                    hovertemplate="<b>%{text}</b><br>5D: %{y:.2f}%<br>20D: %{x:.2f}%<extra></extra>",
                ), row=2, col=2)
                fig.add_hline(y=0, line_dash="dash", line_color=COLORS["neutral"], line_width=1, row=2, col=2)
                fig.add_vline(x=0, line_dash="dash", line_color=COLORS["neutral"], line_width=1, row=2, col=2)
                fig.update_xaxes(title_text="20日动量(%)", row=2, col=2)
                fig.update_yaxes(title_text="5日动量(%)", row=2, col=2)

        # ── 6. 轮动预警列表 ──
        alert_texts = rotation_alerts if rotation_alerts else ["暂无预警"]
        fig.add_trace(go.Table(
            header=dict(
                values=["<b>轮动预警</b>"],
                fill_color=COLORS["danger"] if rotation_alerts else COLORS["neutral"],
                font=dict(color="white", size=12),
                align="left",
            ),
            cells=dict(
                values=[[f"▸ {a}" for a in alert_texts]],
                fill_color=[[COLORS["background"] if i % 2 == 0 else "white" for i in range(len(alert_texts))]],
        ...
        # Layout
        style_label = "成长领先" if style_signal > 0 else "价值领先" if style_signal < 0 else "均衡"
        size_label = "大盘领先" if size_signal > 0 else "小盘领先" if size_signal < 0 else "均衡"
        _color_text_sec = COLORS["text_secondary"]
        title_text = (
            f"AiStock V11 行业/风格/规模轮动仪表板<br>"
            f"<span style='font-size:13px;color:{_color_text_sec}'>"
            f"风格: <b>{style_label}</b> | 规模: <b>{size_label}</b> | "
            f"预警: {len(rotation_alerts)}条</span>"
        )
        _apply_layout(fig, title_text, height=900)
        _apply_axes(fig)
