...
class DataLoader:
    ...
    def __init__(
        ...
        db_pe: Optional[DatabaseService] = None,
        ...
        self.external_api = ExternalAPI(config_service)
    ...
    def _load_stock_from_db(self, code: str, min_days: int) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            query = f'''
                ...
                LIMIT {min_days}
            '''
            records = self.db_main.execute_query(query)
    ...
    def load_all_macro(self) -> Dict[str, Any]:
        """
        加载所有宏观指标数据
        ...
        """
        macro_data = {}
        macro_configs = self.config.get('macro_indicators', {})
        
        if not macro_configs:
            logger.warning("⚠️ 未找到宏观指标配置")
            return macro_data
        
        for name, config in macro_configs.items():
            ...
            # 根据数据源选择加载方式
            if source == 'external':
                value = self.external_api.get_external_futures(code)
            else:
                value = self._load_macro_from_db(code)
            
            if value is not None:
                macro_data[name] = value
        
        logger.info(f"✅ 加载完成 {len(macro_data)} 个宏观指标")
        return macro_data
    
    def _load_macro_from_db(self, code: str, days: int = 60) -> Optional[float]:
        """从数据库加载宏观指标"""
        if self.db_main is None:
            return None
        ...
        try:
            ...
            logger.error(f"❌ 数据库加载宏观指标失败 {code}: {e}")
            return None
    ...
    def _load_financial_from_db(self, code: str) -> Optional[Dict]:
        ...
        try:
            query = f'''
                SELECT revenue_growth, profit_growth, roe, gross_margin, debt_ratio, pe_ttm
