...
StockLists =  ['601162']#pd.read_sql('StockLists', engS).code.tolist()
    ...
    try:
        for i, StockCode in enumerate(StockLists):
            ...
            if StockCode[:2] == '60':
                start = 23500
                while start >= 0:
                    df = api.to_df(api.get_security_bars(Cate, 1, StockCode, start, 500))
                    start = start - 500
                ...
                start = 23500
                while start >= 0:
                    df = api.to_df(api.get_security_bars(Cate, 0, StockCode, start, 500))
                    start = start - 500
                    StockData = StockData.append(df)
