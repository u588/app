...
with api.connect('119.147.212.81', 7709):
    try:
        for i, IndexCode in enumerate(IndexLists):
            ...
            if IndexCode[:2] == '39':
                IndexData = api.to_df(api.get_index_bars(Cate, 0, IndexCode, 0, 48))
                DayUp = IndexData.head(1)['datetime'].tolist()[0]
                Day = pd.read_sql(IndexCode, eng).tail(1)['datetime'].tolist()[0]
                ...
                IndexData = api.to_df(api.get_index_bars(Cate, 1, IndexCode, 0, 48))
                DayUp = IndexData.head(1)['datetime'].tolist()[0]
                Day = pd.read_sql(IndexCode, eng).tail(1)['datetime'].tolist()[0]
