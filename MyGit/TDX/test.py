...
with api.connect('119.147.212.81', 7709):
    try:
        for i, StockCode in enumerate(StockLists):
            ...
            if StockCode >= '600000':
                StockData = api.to_df(api.get_security_bars(Cate, 1, StockCode, 0, 96))
                DayUp = StockData.head(1)['datetime'].tolist()[0]
                try:
                    Day = pd.read_sql(StockCode, eng).tail(1)['datetime'].tolist()[0]
                    if DayUp > Day:
                ...
                pass
                # StockData = api.to_df(api.get_security_bars(Cate, 0, StockCode, 0, 48))
                # DayUp = StockData.head(1)['datetime'].tolist()[0]
                # try:
                #     Day = pd.read_sql(StockCode, eng).tail(1)['datetime'].tolist()[0]
                #     if DayUp > Day:
