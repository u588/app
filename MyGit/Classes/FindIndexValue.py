...
for i, day in enumerate(days):
    print ('Day', day, i, '/', len(days))
    try:
        IndexData = pd.read_csv('f:/IndexsSet/I' + day +'Descri.csv', dtype={'index_code':object})
...
#控制入选成分股
        ...
        MergB.to_csv('f:/IndexsSet/I' + day +'Up.csv', encoding='utf-8')
        print('MergB saved.')

        MergS = pd.merge(MergS, df, on='index_code')
        MergS.set_index('index_code', inplace=True)
        MergS.to_csv('f:/IndexsSet/I' + day +'Down.csv', encoding='utf-8')
