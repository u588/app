...
ThisMon = Today-Sixday

tradeDates = pd.date_range(ThisMon, periods=5).strftime('%Y%m%d').tolist()


for i, tradeDate in enumerate(tradeDates):
	try:
#		print ('MoneyFlow', i, '/', len(tradeDates))
		MoneyFlow = pro.hsgt_top10(trade_date=tradeDate)
		MoneyFlow.set_index(['trade_date'], inplace=True)
		pd.io.sql.to_sql(MoneyFlow, 'HsgtFlow', eng, if_exists='append')
#		print ('MoneyFlow for all stocks got.')
		time.sleep(0.2)
	except:
		pass
#	if i>1:
