...
__font_properties__= font_manager.FontProperties(fname='C:/Windows/Fonts/simfang.ttf')
...
class 分时价格子图:
	...
	def 平面初始化(self, 图片对象, 子图偏移, 全图大小, sharex):
		...
		指数平面= 图片对象.add_axes(布局参数, facecolor='none', label='指数平面', sharex=sharex)	
		...
		# XXX: 指数与个股布局参数一样的话，label 一定要设成不一样，见 add_axes() 官方文档。
		个股平面= 图片对象.add_axes(布局参数, facecolor='none', label='个股平面', sharex=sharex)
