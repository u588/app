"""
AiStock V10 DatabaseReader — PostgreSQL 估值数据读取器 (配置驱动)

V10 关键变更:
  - 构造函数接受 ConfigService, 从 database.yaml 读取连接参数
  - 无 ConfigService 时保留 V9 行为 (从 global_settings 或默认值)
  - 保留全部 V9 功能
...
# ─── 全局配置 (V9 回退: 从 global_settings 获取或使用默认值) ────────────────────
try:
...
class DatabaseReader:
    """
    PostgreSQL 估值数据读取器 (V10 配置驱动)

    V10 变更:
      - 构造函数接受 config_service: ConfigService
      - 从 database.yaml 读取连接参数 (host/port/database/user/password)
      - 自动构建连接 URL
    ...
    def __init__(
        ...
        *,
        ...
        """
        Args:
            config_service: V10 ConfigService 实例
            db_config: 数据库配置 (V9 兼容), 若 None 则从 ConfigService 或 DATABASE_ENGINES 读取
        ...
        # ─── V10: 优先从 ConfigService 加载配置 ──────────────────────────
        if config_service is not None and ConfigService is not None:
            logger.info("DatabaseReader: 使用 ConfigService 加载配置")
    ...
    def _build_config_from_yaml(yaml_section: Dict[str, Any]) -> Dict[str, Any]:
        """
        将 database.yaml 中的配置节转换为 SQLAlchemy 配置
