"""
AiStock V8 DataLoaderService — 数据加载编排服务
...
class LoaderConfig:
    ...
    # ─── 指数K线配置 ─────────────────────────────────────────────────────
    index_codes: List[Dict[str, str]] = field(default_factory=lambda: [
        {"code": "000001", "market_type": MarketType.INDEX_SH, "name": "上证指数"},
        {"code": "399001", "market_type": MarketType.INDEX_SZ, "name": "深证成指"},
        {"code": "399006", "market_type": MarketType.INDEX_SZ, "name": "创业板指"},
        {"code": "000300", "market_type": MarketType.INDEX_SH, "name": "沪深300"},
        {"code": "000905", "market_type": MarketType.INDEX_SH, "name": "中证500"},
        {"code": "000852", "market_type": MarketType.INDEX_SH, "name": "中证1000"},
    ])
    index_bars_count: int = 800

    # ─── 期货K线配置 ─────────────────────────────────────────────────────
    future_codes: List[Dict[str, str]] = field(default_factory=lambda: [
        {"code": "IFM0",  "market_type": MarketType.FUTURE_ZJ, "name": "沪深300主力"},
        {"code": "ICM0",  "market_type": MarketType.FUTURE_ZJ, "name": "中证500主力"},
        {"code": "IMM0",  "market_type": MarketType.FUTURE_ZJ, "name": "中证1000主力"},
        {"code": "IHM0",  "market_type": MarketType.FUTURE_ZJ, "name": "上证50主力"},
        {"code": "TFM0",  "market_type": MarketType.FUTURE_ZJ, "name": "5年期国债主力"},
        {"code": "TM0",   "market_type": MarketType.FUTURE_ZJ, "name": "30年期国债主力"},
        {"code": "CUM0",  "market_type": MarketType.FUTURE_SH, "name": "沪铜主力"},
        {"code": "ALM0",  "market_type": MarketType.FUTURE_SH, "name": "沪铝主力"},
        {"code": "AUM0",  "market_type": MarketType.FUTURE_SH, "name": "沪金主力"},
        {"code": "AGM0",  "market_type": MarketType.FUTURE_SH, "name": "沪银主力"},
    ])
    future_bars_count: int = 800

    # ─── 期权配置 (用于PCR计算) ──────────────────────────────────────────
    option_underlyings: List[Dict[str, str]] = field(default_factory=lambda: [
        {"underlying": "510050", "market_type": MarketType.OPTION_SH, "name": "50ETF期权"},
        {"underlying": "510300", "market_type": MarketType.OPTION_SH, "name": "300ETF期权(沪)"},
        {"underlying": "159919", "market_type": MarketType.OPTION_SZ, "name": "300ETF期权(深)"},
        {"underlying": "IO",     "market_type": MarketType.OPTION_ZJ, "name": "沪深300股指期权"},
    ])
    option_bars_count: int = 100

    # ─── 海外期货配置 ─────────────────────────────────────────────────────
    overseas_tier: str = "core"    # core / extended / all
    overseas_symbols: Optional[List[str]] = None  # 指定品种, None = 按 tier

    # ─── 辅助数据配置 ─────────────────────────────────────────────────────
    auxiliary_types: List[str] = field(default_factory=lambda: [
        "cftc", "lme_stock", "bond_zh_us", "qvix_50etf", "qvix_300etf",
    ])
    ...
    valuation_days: int = 100

    # ─── 宏观数据配置 ─────────────────────────────────────────────────────
    macro_codes: List[Dict[str, Any]] = field(default_factory=lambda: [
...
class DataLoaderService:
    ...
    def __init__(
        ...
        logger.info(
            "DataLoaderService 初始化完成 — 索引:%d 期货:%d 期权标的:%d",
            len(self._config.index_codes),
            len(self._config.future_codes),
            len(self._config.option_underlyings),
        )
    ...
    def load_section(self, section_name: str) -> Any:
        ...
        if self._config.enable_cache:
            ...
            ttl = SECTION_TTL.get(section_name, 300)
    ...
    def load_option_data_for_pcr(
        ...
        for cfg in self._config.option_underlyings:
    ...
    def _load_index_data(self) -> Dict[str, pd.DataFrame]:
        ...
        total = len(self._config.index_codes)
        for idx, cfg in enumerate(self._config.index_codes):
            code = cfg["code"]
            ...
            try:
                df = self._tdx.get_index_daily(
                    ...
                    count=self._config.index_bars_count,
                )
                result[code] = df
                if df.empty:
    ...
    def _load_futures_data(self) -> Dict[str, pd.DataFrame]:
        ...
        total = len(self._config.future_codes)
        for idx, cfg in enumerate(self._config.future_codes):
            code = cfg["code"]
            name = cfg.get("name", code)
            ...
            try:
                df = self._tdx.get_future_daily(
                    code=code,
                    market_type=market_type,
                    count=self._config.future_bars_count,
                )
                result[code] = df
                if df.empty:
    ...
    def _load_option_data(self) -> Dict[str, Dict[str, Any]]:
        ...
        total = len(self._config.option_underlyings)
        for idx, cfg in enumerate(self._config.option_underlyings):
    ...
    def _load_overseas_futures(self) -> Dict[str, Dict[str, Any]]:
        ...
        # 确定加载范围
        if self._config.overseas_symbols:
            symbols = self._config.overseas_symbols
        elif self._config.overseas_tier == "all":
            symbols = ALL_SYMBOLS
        elif self._config.overseas_tier == "core":
    ...
    def _load_auxiliary_data(self) -> Dict[str, pd.DataFrame]:
        ...
        total = len(self._config.auxiliary_types)
        for idx, data_type in enumerate(self._config.auxiliary_types):
            try:
    ...
    def _load_valuation_data(self) -> Dict[str, pd.DataFrame]:
        ...
        try:
            batch_result = self._db.get_index_pe_batch(self._config.valuation_codes)
            result = batch_result
        except Exception as e:
            logger.error("批量估值数据加载失败, 回退逐条加载: %s", e)
            for code in self._config.valuation_codes:
                try:
                    df = self._db.get_index_pe(code, self._config.valuation_days)
                    result[code] = df
    ...
    def _load_macro_data(self) -> Dict[str, pd.DataFrame]:
        ...
        total = len(self._config.macro_codes)
        for idx, cfg in enumerate(self._config.macro_codes):
            code = cfg["code"]
            name = cfg.get("name", code)
            ...
            try:
                df = self._tdx.get_macro_data(
                    code=code,
                    market=market,
                    count=self._config.macro_count,
                )
                result[code] = df
                if df.empty:
