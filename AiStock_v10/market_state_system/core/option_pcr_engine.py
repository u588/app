...
_MARKET_CZCE_OPT = 4     # 郑商所商品期权 (TDX期权market_code=4, 非期货28)
_MARKET_GZ_OPT = 67      # 广州期权
...
class OptionPCREngine:
    ...
    def __init__(
        ...
        contract_manager: Any = None,  # V9: ContractManager 注入
    ) -> None:
        """初始化 PCR 引擎
        ...
        Args:
            ...
            contract_manager: ContractManager 实例 (V9: 提供code/code_name双码查询)
        """
        self._tdx = tdx_adapter
        self._config = config
        self._cache = cache_service
        self._logger = logger_instance or logger
        self._contract_mgr = contract_manager  # V9: 合约管理器
    ...
    def calculate_etf_pcr(self, underlying: str) -> PCRResult:
        ...
        try:
            ...
            # 2. 解析合约代码 — V9: 使用 code_name (显示码) 解析
            parse_batch_input = []
            for c in contracts_info:
                # OptionCodeParser 解析 code_name (显示码)
                code_name_for_parse = c.get("code_name", c.get("code", ""))
                if code_name_for_parse:
                    parse_batch_input.append({
                        "code_name": code_name_for_parse,
                        "market_code": market_code,
                    })

            parsed = self._parser.parse_batch(parse_batch_input)

            # 3. 过滤: 只保留属于该标的的合约
            target_parsed = [p for p in parsed if p.underlying == underlying]

            # 4. 获取各合约的 OI/Volume — V9: 使用 code (TDX内部码) 调API
            call_oi, put_oi, call_vol, put_vol = 0, 0, 0, 0
            call_oi_cm, put_oi_cm = 0, 0  # 当月
            adj_count = 0

            for info in target_parsed:
                # V9: 从 code_name 反查 TDX 内部码
                api_code = self._resolve_api_code(info.code_name, contracts_info)
                oi, volume = self._fetch_contract_oi_volume(
                    api_code, market_code,
                )
        ...
        try:
            ...
            for c in contracts_info:
                ...
                if code_name_for_parse:
                    parse_batch_input.append({
                        "code_name": code_name_for_parse,
                        "market_code": self._get_commodity_market_code(variety) or 0,
                    })
    ...
    def _fetch_etf_option_contracts(
        ...
        V9 优先从 ContractManager 代码表获取 (xlsx 数据源),
        代码表不可用时回退到 TDX API 在线获取。

        返回格式统一为:
          {'code': TDX内部码, 'code_name': 显示码, 'market_code': 市场编号}
        ...
        Returns:
            合约字典列表
        """
        # V9: 优先从 ContractManager 的 xlsx 代码表获取
        if self._contract_mgr is not None:
            try:
                contracts = self._fetch_from_code_table(underlying, market_code)
                if contracts:
                    return contracts
            except Exception as e:
                self._logger.debug("ContractManager获取ETF合约失败,回退TDX API: %s", e)
        ...
        try:
            ...
            # V9: 按显示名 (code_name/name 列) 筛选 ETF 期权
            # TDX API 返回的 DataFrame 含 code 和 name 列
            # name 列含显示码如 510050C3A02700
            contracts: List[Dict[str, Any]] = []
            for _, row in df.iterrows():
                code = str(row.get("code", ""))      # TDX内部码
                name = str(row.get("name", ""))       # 显示码 (code_name)
                row_market = row.get("market", market_code)

                if not code or int(row_market) != market_code:
                    continue

                # 用显示名筛选: ETF期权显示名以标的代码开头
                if name and name.startswith(str(underlying)):
                    contracts.append({
                        "code": code,                 # TDX内部码 — 用于API调用
                        "code_name": name,            # 显示码 — 用于OptionCodeParser解析
                        "market_code": market_code,
                    })

            self._logger.debug(
                "ETF合约获取(TDX API): %s market=%d → %d 合约",
    ...
    def _fetch_from_code_table(
        self,
        underlying: str,
        market_code: int,
    ) -> List[Dict[str, Any]]:
        """V9: 从 ContractManager 代码表获取期权合约 (xlsx 数据源)

        Args:
            underlying:   标的代码或品种代码
            market_code:  市场编号

        Returns:
            合约字典列表 (含 code + code_name)
        """
        from market_state_system.core.contract_manager import ContractInfo

        contracts: List[Dict[str, Any]] = []

        # 从 _option_by_market 或 _option_by_underlying 查找
        option_list = self._contract_mgr._option_by_underlying.get(underlying, [])
        if not option_list:
            # 也尝试从 market_code 筛选
            option_list = self._contract_mgr._option_by_market.get(market_code, [])

        for info in option_list:
            if not isinstance(info, ContractInfo):
                continue
            # 按标的筛选
            if info.market_code != market_code:
                continue

            # ETF期权: variety 字段是标的代码 (如 '510050')
            # CFFEX期权: variety 字段是品种代码 (如 'IO')
            if info.variety != underlying:
                continue

            contracts.append({
                "code": info.code,             # TDX内部码
                "code_name": info.code_name,   # 显示码
                "market_code": info.market_code,
            })

        self._logger.debug(
            "ETF合约获取(代码表): %s market=%d → %d 合约",
            underlying, market_code, len(contracts),
        )
        return contracts
    ...
    def _fetch_cffex_option_contracts(
        ...
        V9: 优先从 ContractManager 代码表获取, 回退到 TDX API。
        ...
        if self._contract_mgr is not None:
            try:
                contracts = self._fetch_from_code_table(variety, _MARKET_CFFEX)
                if contracts:
                    return contracts
            except Exception as e:
                self._logger.debug("ContractManager获取CFFEX合约失败: %s", e)
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                row_market = row.get("market", _MARKET_CFFEX)

                if not code or int(row_market) != _MARKET_CFFEX:
                    continue

                # CFFEX: 用显示名筛选 (含品种代码如 HO2602-P-2650)
                if name and name.startswith(variety):
                    contracts.append({
                        "code": code,
                        "code_name": name,
                        "market_code": _MARKET_CFFEX,
    ...
    def _fetch_commodity_option_contracts(
        ...
        if self._contract_mgr is not None:
            try:
                contracts = self._fetch_from_code_table(variety, market_code)
                if contracts:
                    return contracts
            except Exception as e:
                self._logger.debug("ContractManager获取商品期权合约失败: %s", e)
    ...
    def _resolve_api_code(
        self,
        code_name: str,
        contracts_info: List[Dict[str, Any]],
    ) -> str:
        """V9: 从 code_name (显示码) 反查 code (TDX内部码)

        用于: 将 OptionCodeParser 解析出的 code_name 转换为 TDX API 所需的 code。

        查找优先级:
          1. 从 contracts_info 列表中查找 (来自代码表或TDX API)
          2. 从 ContractManager 的映射表中查找
          3. 回退: 使用 code_name 本身 (兼容旧格式)

        Args:
            code_name:      显示码 (如 '510050C3A02700', 'HO2602-P-2650')
            contracts_info: 合约信息列表 (含 code + code_name)

        Returns:
            TDX 内部码 (如 '10009633', 'HO8Q0438')
        """
        # 1. 从 contracts_info 中查找
        for c in contracts_info:
            if c.get("code_name") == code_name and c.get("code"):
                return c["code"]

        # 2. 从 ContractManager 查找
        if self._contract_mgr is not None:
            api_code = self._contract_mgr.lookup_code_by_code_name(code_name)
            if api_code:
                return api_code

        # 3. 回退: 使用 code_name 本身
        self._logger.debug("无法反查TDX内部码, 使用显示码: %s", code_name)
        return code_name
    ...
    # 商品品种 → TDX 期权市场编号 映射
    # 注意: TDX 扩展端口的期权 market_code 与期货不同!
    #   郑商所期权=4 (期货=28), 大商所期权=5, 上期所期权=6, 广州期权=67
    _COMMODITY_MARKET_CODE_MAP: Dict[str, int] = {
        # SHFE 上期所商品期权 → market_code=6
        "CU": 6, "AG": 6, "AU": 6, "AL": 6,
        "ZN": 6, "RB": 6, "NI": 6, "RU": 6,
        "SN": 6, "PB": 6, "SS": 6, "WR": 6,
        "HC": 6, "BU": 6, "FU": 6, "SP": 6,
        "NR": 6, "BC": 6, "AO": 6, "BR": 6,
        "EC": 6,
        # DCE 大商所商品期权 → market_code=5
        "M": 5, "Y": 5, "P": 5, "C": 5,
        "I": 5, "JM": 5, "J": 5, "A": 5,
        "B": 5, "BB": 5, "FB": 5, "L": 5,
        "V": 5, "PP": 5, "EG": 5, "EB": 5,
        "JD": 5, "PG": 5, "RR": 5, "LH": 5,
        # CZCE 郑商所商品期权 → market_code=4 (NOT 28, 28是期货)
        "CF": 4, "SR": 4, "TA": 4, "MA": 4,
        "AP": 4, "OI": 4, "FG": 4, "RS": 4,
        "RM": 4, "ZC": 4, "WH": 4, "PM": 4,
        "RI": 4, "JR": 4, "LR": 4, "SF": 4,
        "SM": 4, "CY": 4, "CJ": 4, "PF": 4,
        "SH": 4, "UR": 4, "SA": 4, "PK": 4,
        # GZFE 广州期货期权 → market_code=67
        "LC": 67, "SI": 67, "PS": 67, "PD": 67, "PT": 67,
    }
    ...
    def _get_commodity_market_code(variety: str) -> Optional[int]:
        ...
        TDX 扩展端口期权 market_code (注意与期货不同):
          SHFE 品种 → 6
          DCE 品种  → 5
          CZCE 品种 → 4 (期货是28, 期权是4)
          GZFE 品种 → 67
        """
        return OptionPCREngine._COMMODITY_MARKET_CODE_MAP.get(variety)
