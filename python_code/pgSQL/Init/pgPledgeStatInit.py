...
for i, tradeDate in enumerate(tradeDates):
	try:
		print ('Pledge', i, '/', len(tradeDates))
		Pledge = pro.pledge_stat(trade_date=tradeDate)
		Pledge.set_index(['trade_date'], inplace=True)
		pd.io.sql.to_sql(Pledge, 'HsgtFlow', eng, if_exists='append')
#		print ('Pledge for all stocks got.')
		time.sleep(0.2)
	except:
		pass
#	if i>1:
