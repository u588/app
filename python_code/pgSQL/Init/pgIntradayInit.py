...
eng = create_engine('postgresql+psycopg2://sa:11111111@10.145.254.56/db')
...
def stockPriceIntraday(ticker):
 ...
 #   print(intraday).head()
    ...
    #     stamp = history.tail(1)['trade_date'].tolist()[0]
    #     intraday = intraday[(intraday['trade_date']>stamp)]
 #   intraticker
    pd.io.sql.to_sql(intraday, ticker, eng, if_exists='replace')
