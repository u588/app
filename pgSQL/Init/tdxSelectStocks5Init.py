...
with api.connect('119.147.212.81', 7709):
    try:
        for i, StockCode in enumerate(StockLists):
            ...
            if StockCode <'600000':
                StockData = api.to_df(api.get_security_bars(Cate, 0, StockCode, 48, 240))
                StockData.set_index('datetime', inplace=True)
                StockData.to_sql(StockCode, eng, if_exists='append')
                print(StockCode,'saved to sql !')

            elif StockCode>='600000':
                StockData = api.to_df(api.get_security_bars(Cate, 1, StockCode, 48, 240))
                StockData.set_index('datetime', inplace=True)
                StockData.to_sql(StockCode, eng, if_exists='append')
                print(StockCode,'saved to sql !')

            # elif StockCode>'600525' and StockCode<'604000':
            #     StockData = api.to_df(api.get_security_bars(Cate, 1, StockCode, 0, 288))
            #     StockData.set_index('datetime', inplace=True)
            #     StockData.to_sql(StockCode, eng, if_exists='append')
            #     print(StockCode,'saved to sql !')                
